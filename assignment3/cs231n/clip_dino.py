...
def get_similarity_no_loop(text_features, image_features):
    ...
    M,D = image_features.shape
    cos = torch.nn.CosineSimilarity(dim=2)
    text_features = text_features.reshape(N,1,D)
    image_features = image_features.reshape(1,M,D)
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    best_class_indices = similarity.argmax(dim=0)

    pred_classes = [class_texts[idx.item()] for idx in best_class_indices]
...
class CLIPImageRetriever:
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        top_indices = torch.topk(similarity.squeeze(0), k=k).indices
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return top_indices
