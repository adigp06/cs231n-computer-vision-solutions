...
def sim(z_i, z_j):
    ...
    norm_dot_product = z_i @ z_j.T
    norm_dot_product = norm_dot_product / (torch.linalg.norm(z_i)*torch.linalg.norm(z_j))                                                                  #
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for n in range(2*N):
          if n!=k:
            denominator += torch.exp(sim(z_k,out[n])/tau)
            
          numerator = torch.exp(sim(z_k, z_k_N)/tau)
          loss_k_kN = -torch.log(numerator/denominator)
          
        denominator2 = 0.0
        for m in range(2*N):
          if m != k:
            denominator2 += torch.exp(sim(z_k_N,out[m])/tau)
          
          numerator2 = torch.exp(sim(z_k_N,z_k)/tau)
          loss_kN_k = -torch.log(numerator2/denominator2)
...
def compute_sim_matrix(out):
    ...
    dotproduct = out @ out.T
    sim_matrix = dotproduct / (torch.linalg.norm(out,dim=1,keepdims=True)*torch.linalg.norm(out.T,dim=0,keepdims=True))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos = sim_positive_pairs(out_left,out_right)
    positive = torch.cat([pos,pos],dim=0)
