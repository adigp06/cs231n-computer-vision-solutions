...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        for b in range(N):
          for i in range(S):
            for j in range(D):
              output[b,i,j] = x[b,i,j] + self.pe[0,i,j]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        Q = Q.reshape(N,S,H,E//H).permute(0,2,1,3)
        K = K.reshape(N,T,H,E//H).permute(0,2,1,3)
        V = V.reshape(N,T,H,E//H).permute(0,2,1,3)

        e = torch.matmul(Q,K.permute(0,1,3,2))/((E//H) ** 0.5)

        if attn_mask is not None:
          e = e.masked_fill(attn_mask==0,float('-inf'))

        a = torch.softmax(e,dim=-1) #alongT
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        out1 = self.norm_self(src + self.dropout_self(attn_out))
        ffn_out = self.ffn(out1)
        out = self.norm_ffn(out1 + self.dropout_ffn(ffn_out))
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return src
